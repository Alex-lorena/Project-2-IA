...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentFactor = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Interates over the elimination order
        for varEliminate in eliminationOrder:
            currentFactor, joinedFactor = joinFactorsByVariable(currentFactor, varEliminate)

            # Check if the joine factor has more than one uncondiioned variable
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminatedFactor = eliminate(joinedFactor, varEliminate)
                # If the elimination results in a valid factor, add it to the current factors list
                if eliminatedFactor:
                    currentFactor.append(eliminatedFactor)

        finalJointFactor = joinFactors(currentFactor)

        normalizedDistribution = normalize(finalJointFactor)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Get pacman's current position and jail position once, as they won't change during interation
        pacmanPos = gameState.getPacmanPosition()
        jailPos = self.getJailPosition()

        # Calculate and update beliefs based on observaion probabilities
        for ghostPos in self.allPositions:
            observationProb = self.getObservationProb(observation, pacmanPos, ghostPos, jailPos)
            updatedBeliefs[ghostPos] = self.beliefs[ghostPos] * observationProb

        self.beliefs.update(updatedBeliefs)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        # Pre-calculate belief values for each old postion to avoid redudant calculations
        beliefValue = {oldPos: self.beliefs[oldPos] for oldPos in self.allPositions}

        # Compute contribution to new position beliefs
        for oldPos, oldBeliefs in beliefValue.items():
            newPosDistribution = self.getPositionDistribution(gameState, oldPos) 

            # Disribute the belief from the old position to the new position
            for newPos, transitionProb in newPosDistribution.items():
                predictedBeliefs[newPos] += oldBeliefs * transitionProb
